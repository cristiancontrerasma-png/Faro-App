...
// ══════════════════════════════════════════════════════════
// FARO v2.1 — Vercel + Claude artifact (auto-detecta storage)
// ══════════════════════════════════════════════════════════

// ─── STORAGE: funciona en Vercel Y en Claude artifact ────
const S = {
...
// ─── UTILS ────────────────────────────────────────────────
const fmt      = n => "$"+Math.round(Math.abs(Number(n)||0)).toLocaleString("es-CL");
...
const calcTension = (comp,ing,gast) => {
  if(!ing) return 0;
  const tot=comp.filter(c=>c.activo).reduce((s,c)=>s+Number(c.monto||0),0);
  return Math.min(100,Math.round(((tot+gast)/ing)*70+(tot>ing*0.7?20:0)));
};
const tensionInfo = v => {
...
// ══════════════════════════════════════════════════════════
// APPS SCRIPT v2.1 — regex multilinea, fechas REALES
// ══════════════════════════════════════════════════════════
const APPS_SCRIPT_CODE = `// FARO Gmail Script v2.1 — fechas reales (no inventadas)
const EMPRESAS = [
  {key:"enel",          nombre:"Enel (Luz)",      query:"from:enelchile.cl OR subject:(cuenta enel)"},
  ...
  {key:"movistar",      nombre:"Movistar",         query:"from:movistar.cl"},
  {key:"claro",         nombre:"Claro",            query:"from:clarochile.cl"},
  {key:"scotiabank",    nombre:"Scotiabank",       query:"from:scotiabank.cl dividendo"},
  {key:"gastos_comunes",nombre:"Gastos Comunes",   query:"subject:(gastos comunes) newer_than:45d"},
];
const PATRONES_MONTO = [
  /cu[aá]nto debo pagar[\\s\\S]{0,30}\\$?\\s*([\\d.]+)/i,
  /total a pagar[\\s\\S]{0,30}\\$?\\s*([\\d.]+)/i,
  /monto a pagar[\\s\\S]{0,30}\\$?\\s*([\\d.]+)/i,
  /total[:\\s]*\\$?\\s*([\\d.]+)/i,
  /\\$\\s*([\\d.]{4,})/,
];
// Regex multilinea — captura fecha aunque esté en línea siguiente
const PATRONES_FECHA = [
  /fecha de vencimiento[\\s\\S]{0,50}(\\d{1,2}[-\\/]\\d{1,2}[-\\/]\\d{2,4})/i,
  /vencimiento[\\s\\S]{0,50}(\\d{1,2}[-\\/]\\d{1,2}[-\\/]\\d{2,4})/i,
  /vence[\\s\\S]{0,50}(\\d{1,2}[-\\/]\\d{1,2}[-\\/]\\d{2,4})/i,
  /fecha de pago[\\s\\S]{0,50}(\\d{1,2}[-\\/]\\d{1,2}[-\\/]\\d{2,4})/i,
  /pagar antes del[\\s\\S]{0,50}(\\d{1,2}[-\\/]\\d{1,2}[-\\/]\\d{2,4})/i,
];
function extraerMonto(txt) {
  for (const p of PATRONES_MONTO) {
    const m=txt.match(p); if(m){ const n=parseInt(m[1].replace(/[.\\s]/g,"")); if(n>500&&n<10000000) return n; }
  }
  return null;
}
function extraerDia(txt) {
  for (const p of PATRONES_FECHA) {
    const m=txt.match(p);
    if(m){ const d=parseInt(m[1].split(/[-\\/]/)[0]); if(d>=1&&d<=31){ Logger.log("Fecha real: "+m[1]+" → día "+d); return d; } }
  }
  Logger.log("Sin fecha detectada — se mantiene fecha original"); return null;
}
function buscarBoletas() {
  const res=[]; const hoy=new Date();
  EMPRESAS.forEach(emp=>{
    try {
      GmailApp.search(emp.query+" newer_than:60d",0,3).forEach(th=>{
        const msg=th.getMessages().pop();
        const diff=(hoy-msg.getDate())/(864e5); if(diff>45) return;
        const txt=msg.getSubject()+"\\n"+msg.getPlainBody();
        const monto=extraerMonto(txt);
        if(monto&&!res.some(r=>r.key===emp.key))
          res.push({key:emp.key,nombre:emp.nombre,monto,diaVence:extraerDia(txt),fechaEmail:Utilities.formatDate(msg.getDate(),"America/Santiago","dd/MM/yyyy"),asunto:msg.getSubject().substring(0,80)});
      });
    } catch(e){ Logger.log("Error "+emp.nombre+": "+e.message); }
  });
  try {
    const ss=SpreadsheetApp.getActiveSpreadsheet();
    const h=ss.getSheetByName("FARO_Boletas")||ss.insertSheet("FARO_Boletas");
    h.clearContents(); h.appendRow(["key","nombre","monto","diaVence","fechaEmail","asunto","ts"]);
    res.forEach(r=>h.appendRow([r.key,r.nombre,r.monto,r.diaVence||"",r.fechaEmail,r.asunto,new Date().toISOString()]));
    Logger.log("✅ "+res.length+" boletas con fechas REALES");
  } catch(e){ Logger.log(JSON.stringify(res,null,2)); }
  return res;
}
function doGet() {
  try {
    const h=SpreadsheetApp.getActiveSpreadsheet().getSheetByName("FARO_Boletas");
    if(!h){ buscarBoletas(); return ContentService.createTextOutput(JSON.stringify({ok:true,data:[]})).setMimeType(ContentService.MimeType.JSON); }
    const boletas=h.getDataRange().getValues().slice(1).filter(r=>r[0]&&Number(r[2])>0).map(r=>({key:r[0],nombre:r[1],monto:Number(r[2]),diaVence:r[3]?Number(r[3]):null,fechaEmail:r[4],asunto:r[5]}));
    return ContentService.createTextOutput(JSON.stringify({ok:true,data:boletas,timestamp:new Date().toISOString()})).setMimeType(ContentService.MimeType.JSON);
...
function activarSincronizacionDiaria() {
  ...
  ScriptApp.newTrigger("buscarBoletas").timeBased().everyDays(1).atHour(8).create();
  Logger.log("✅ Sync diario a las 8am activado");
}`;
    ...
    <div style={{background:isDark?"linear-gradient(135deg,#064E3B,#065F46)":"linear-gradient(135deg,#ECFDF5,#D1FAE5)",border:`1px solid ${t.green}44`,borderRadius:18,padding:"16px",marginBottom:16,boxShadow:t.shadow}}>
      <div style={{display:"flex",gap:10,alignItems:"flex-start",marginBottom:14}}>
        ...
        <div><div style={{fontSize:14,fontWeight:800,color:isDark?"#ECFDF5":"#064E3B"}}>FARO detectó {boletas.length} boleta{boletas.length>1?"s":""} nueva{boletas.length>1?"s":""}</div>
          <div style={{fontSize:12,color:t.t2}}>Monto y fecha del email original</div></div>
      </div>
      {boletas.map(b=>(
        <div key={b.key} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 13px",background:isDark?"rgba(255,255,255,0.06)":"rgba(255,255,255,0.7)",borderRadius:12,marginBottom:8,border:`1px solid ${t.green}22`}}>
          <div>
            <div style={{fontSize:13,fontWeight:700,color:isDark?"#ECFDF5":"#064E3B"}}>{b.nombre}</div>
            {b.diaVence
              ? <div style={{fontSize:11,color:t.green,fontWeight:600}}>📅 Vence día {b.diaVence} (del email)</div>
              : <div style={{fontSize:11,color:t.gold}}>📅 Fecha sin cambios (no detectada)</div>}
          </div>
          <div style={{fontSize:16,fontWeight:800,color:t.green}}>{fmt(b.monto)}</div>
        </div>
      ))}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:4}}>
        <button onClick={onDescartar} style={{padding:"11px",borderRadius:12,background:"transparent",color:t.t2,border:`1px solid ${t.border}`,cursor:"pointer",fontFamily:"inherit",fontWeight:600,fontSize:14}}>Ignorar</button>
        <button onClick={()=>onConfirmar(boletas)} style={{padding:"11px",borderRadius:12,background:`linear-gradient(135deg,${t.green},#047857)`,color:"#fff",border:"none",cursor:"pointer",fontFamily:"inherit",fontWeight:800,fontSize:14}}>✓ Cargar en FARO</button>
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div style={{background:isDark?"linear-gradient(145deg,#0F2D52,#1E3A5F)":"linear-gradient(145deg,#0F4C81,#1565C0)",borderRadius:22,padding:"20px",marginBottom:14,boxShadow:"0 8px 32px rgba(15,76,129,0.3)"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}>
          ...
          <div style={{fontSize:12,fontWeight:700,color:disponible>=0?"#A7F3D0":"#FCA5A5"}}>{disponible>=0?`Quedan ${fmt(disponible)}`:`Faltan ${fmt(Math.abs(disponible))}`}</div>
        </div>
          ...
          {proximos.map((c,i)=>{
            ...
            return(
              <div key={c.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:i<proximos.length-1?`1px solid ${t.border}`:"none"}}>
                ...
                <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:t.t1}}>{c.nombre}</div>
                  <div style={{fontSize:11,color:urg?t.red:prox?t.gold:t.t3,fontWeight:urg||prox?700:400}}>{urg?`🚨 Vence en ${c.dias}d`:prox?`⏰ ${c.dias} días`:`Día ${c.dia} del mes`}</div></div>
...
function CompromisosView({ data, setData, t, isDark }) {
  ...
  const add   = ()=>{ if(!nuevo.nombre||!nuevo.monto) return; setData(d=>({...d,compromisos:[...d.compromisos,{...nuevo,id:"c_"+Date.now(),monto:Number(nuevo.monto),pagado:false}]})); setNuevo({nombre:"",icon:"📋",color:"#3B82F6",monto:"",dia:1,tipo:"fijo",banco:"",activo:true}); setAdd(false); };
  ...
  const handleFoto=async e=>{
    ...
    reader.onload=async ev=>{
      ...
      try {
        const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:200,messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:file.type||"image/jpeg",data:b64}},{type:"text",text:'Extrae SOLO los datos REALES de esta boleta. JSON puro: {"monto":663141,"dia_vence":8,"fecha_texto":"08-06-2026"}. Si NO encuentras fecha real pon dia_vence:null. NUNCA inventes.'}]}]})});
        const d=await r.json();
        const res=JSON.parse((d.content?.[0]?.text||"{}").replace(/```json|```/g,"").trim());
        setScanResult({compId:scanId,monto:res.monto||null,dia:res.dia_vence||null,fechaTexto:res.fecha_texto||null});
    ...
    return(
      <div style={{background:t.card,borderRadius:16,padding:"14px",marginBottom:10,border:`1.5px solid ${c.pagado?t.green+"33":isEd?t.accent+"44":t.border}`,boxShadow:t.shadow}}>
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{display:"flex",justifyContent:"space-between"}}>
              <div><div style={{fontSize:14,fontWeight:700,color:t.t1}}>{c.nombre}</div>
                <div style={{fontSize:11,color:t.t3}}>{c.tipo==="variable"?"⚡ Variable":"📌 Fijo"} · Día {c.dia}{c.banco?` · ${c.banco}`:""}</div></div>
              <div style={{textAlign:"right",flexShrink:0}}>
                <div style={{fontSize:17,fontWeight:800,color:c.pagado?t.green:c.monto?t.t1:t.t3}}>{c.monto?fmt(c.monto):<span style={{fontSize:12,color:t.t3}}>Sin monto</span>}</div>
        ...
        <div style={{display:"flex",gap:7,marginTop:11}}>
          <button onClick={()=>toggle(c.id)} style={{flex:1,padding:"8px",borderRadius:10,border:`1px solid ${c.pagado?t.green:t.border}`,background:c.pagado?t.greenBg:t.muted,color:c.pagado?t.green:t.t2,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>{c.pagado?"✅ Pagado":"Marcar pagado"}</button>
        ...
        {scanResult&&scanResult.compId===c.id&&(
          <div style={{marginTop:10,background:isDark?"#0C2A1A":"#ECFDF5",border:`1px solid ${t.green}44`,borderRadius:12,padding:"12px"}}>
            <div style={{fontSize:12,fontWeight:700,color:t.green,marginBottom:8}}>📋 Extraído de la boleta:</div>
            <div style={{display:"flex",gap:10,marginBottom:10}}>
              {scanResult.monto&&<div style={{flex:1,background:t.card,borderRadius:9,padding:"8px 10px",textAlign:"center"}}><div style={{fontSize:10,color:t.t3,marginBottom:2}}>MONTO</div><div style={{fontSize:15,fontWeight:800,color:t.t1}}>{fmt(scanResult.monto)}</div></div>}
              <div style={{flex:1,background:t.card,borderRadius:9,padding:"8px 10px",textAlign:"center"}}><div style={{fontSize:10,color:t.t3,marginBottom:2}}>VENCIMIENTO</div>
                <div style={{fontSize:14,fontWeight:800,color:scanResult.dia?t.green:t.gold}}>{scanResult.dia?`Día ${scanResult.dia}`:"No detectado"}</div>
                {scanResult.fechaTexto&&<div style={{fontSize:10,color:t.t3}}>{scanResult.fechaTexto}</div>}
              </div>
            </div>
            {!scanResult.dia&&<div style={{fontSize:11,color:t.gold,marginBottom:8,padding:"6px 9px",background:t.goldBg,borderRadius:8}}>⚠️ Fecha no detectada — se mantiene día {c.dia} original</div>}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              <button onClick={()=>setScanResult(null)} style={{padding:"9px",borderRadius:9,background:t.muted,color:t.t2,border:`1px solid ${t.border}`,cursor:"pointer",fontFamily:"inherit",fontWeight:600}}>Cancelar</button>
        ...
        {isEd&&(
          <div style={{marginTop:12,padding:"12px",background:t.muted,borderRadius:12,display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
            <div><div style={{fontSize:10,color:t.t3,fontWeight:700,marginBottom:4}}>MONTO ($)</div><input value={c.monto||""} onChange={e=>upd(c.id,{monto:e.target.value})} type="tel" inputMode="numeric" placeholder="0" style={inp}/></div>
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
        <div><div style={{fontSize:21,fontWeight:800,color:t.t1,letterSpacing:-0.5}}>Compromisos 📋</div><div style={{fontSize:12,color:t.t2}}>Todo lo que debes pagar este mes</div></div>
      ...
      {showAdd&&(
        <div style={{background:t.card,borderRadius:16,padding:"16px",border:`1.5px dashed ${t.accent}55`,marginTop:8}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9,marginBottom:10}}>
            ...
            <div><div style={{fontSize:10,color:t.t3,fontWeight:700,marginBottom:4}}>MONTO ($)</div><input value={nuevo.monto} onChange={e=>setNuevo(p=>({...p,monto:e.target.value}))} type="tel" inputMode="numeric" placeholder="0" style={inp}/></div>
          ...
          <div style={{marginBottom:12}}><div style={{fontSize:10,color:t.t3,fontWeight:700,marginBottom:6}}>ÍCONO</div>
            <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>{["📋","🏠","💳","🚗","📱","🎬","💊","🏋️","🐾","📚","✈️","💡","💧","🔥","📡","🔑"].map(ic=><button key={ic} onClick={()=>setNuevo(p=>({...p,icon:ic}))} style={{width:34,height:34,borderRadius:9,border:`2px solid ${nuevo.icon===ic?t.accent:t.border}`,background:nuevo.icon===ic?t.accentBg:t.muted,fontSize:17,cursor:"pointer"}}>{ic}</button>)}</div></div>
...
// ══════════════════════════════════════════════════════════
// PRESUPUESTO — con movimientos del mes + eliminar por ítem
// ══════════════════════════════════════════════════════════
function PresupuestoView({ data, setData, t, isDark }) {
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
        <div><div style={{fontSize:21,fontWeight:800,color:t.t1,letterSpacing:-0.5}}>Presupuesto 🎯</div><div style={{fontSize:12,color:t.t2}}>Cuánto asignas a cada categoría</div></div>
      ...
      <div style={{background:isDark?"linear-gradient(135deg,#064E3B,#065F46)":"linear-gradient(135deg,#ECFDF5,#D1FAE5)",borderRadius:14,padding:"13px 15px",marginBottom:14,border:`1px solid ${t.green}22`}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,textAlign:"center"}}>
          ...
          <div><div style={{fontSize:10,color:isDark?"#6EE7B7":"#059669",fontWeight:700,marginBottom:4}}>GASTADO MES</div><div style={{fontSize:20,fontWeight:800,color:totalGast>totalPres&&totalPres>0?t.red:(isDark?"#ECFDF5":"#064E3B")}}>{fmt(totalGast)}</div></div>
        </div>
                ...
                <div key={cat.id} style={{background:t.card,borderRadius:14,padding:"13px",marginBottom:8,border:`1.5px solid ${over?t.red+"44":isEd?t.accent+"44":t.border}`,boxShadow:t.shadow}}>
                  <div style={{display:"flex",gap:10,alignItems:"center"}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      {pres>0&&<><div style={{height:4,background:t.muted,borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:`${Math.min((gast/pres)*100,100)}%`,background:over?`linear-gradient(90deg,${t.red},#F87171)`:`linear-gradient(90deg,${cat.color},${cat.color}88)`,borderRadius:2}}/></div><div style={{fontSize:10,color:over?t.red:t.t3,marginTop:3,fontWeight:over?700:400}}>{over?`⚠️ Excediste en ${fmt(gast-pres)}`:`${fmt(pres-gast)} disponible`}</div></>}
                    </div>
                  </div>
                  {isEd&&(
                    <div style={{marginTop:10,display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                      <div><div style={{fontSize:10,color:t.t3,fontWeight:700,marginBottom:3}}>MONTO MENSUAL</div><input value={cat.presupuesto||""} onChange={e=>updCat(cat.id,{presupuesto:Number(e.target.value.replace(/\D/g,""))})} onKeyDown={e=>e.key==="Enter"&&setEdit(null)} placeholder="Ej: 120000" style={inp} autoFocus/></div>
      ...
      <div style={{marginTop:20,borderTop:`1px solid ${t.border}`,paddingTop:16}}>
        ...
        {showMovs&&(
          mesG.length===0
            ...
            : [...mesG].sort((a,b)=>new Date(b.fecha)-new Date(a.fecha)).map(g=>{
                ...
                return(
                  <div key={g.id} style={{background:t.card,borderRadius:12,padding:"11px 13px",marginBottom:7,border:`1px solid ${t.border}`,display:"flex",gap:10,alignItems:"center",boxShadow:t.shadow}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{fontSize:10,color:t.t3}}>{g.fecha} · {cat?.nombre||"Otros"}</div>
                    </div>
                    <div style={{fontSize:15,fontWeight:800,color:g.monto>0?t.green:t.red,flexShrink:0,marginRight:4}}>{g.monto>0?"+":""}{fmt(g.monto)}</div>
                    <button onClick={()=>delGasto(g.id)} style={{width:28,height:28,borderRadius:8,border:`1px solid ${t.border}`,background:t.redBg,color:t.red,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>✕</button>
                  </div>
                );
              })
        )}
        {showMovs&&mesG.length>0&&(
          <button onClick={()=>{ if(window.confirm("¿Eliminar TODOS los movimientos del mes?")) setData(d=>({...d,gastos:d.gastos.filter(g=>{ const dd=new Date(g.fecha); return !(dd.getMonth()===NOW.getMonth()&&dd.getFullYear()===NOW.getFullYear()); })})); }} style={{width:"100%",marginTop:8,padding:"10px",borderRadius:11,background:t.redBg,color:t.red,border:`1px solid ${t.red}22`,cursor:"pointer",fontFamily:"inherit",fontSize:12,fontWeight:700}}>
            🗑️ Eliminar todos los movimientos del mes
          </button>
        )}
      </div>
    </div>
...
function AjustesView({ data, setData, t, isDark, onSyncGmail }) {
  ...
  const copiarScript=()=>{ navigator.clipboard?.writeText(APPS_SCRIPT_CODE).then(()=>{ setCop(true); setTimeout(()=>setCop(false),3e3); }); };
  ...
  const sincronizarAhora=async()=>{
    ...
    setSyn(true); setSMsg("🔦 Consultando boletas...");
    try {
      ...
      if(boletas.length>0){ const mapped=boletas.map(b=>({key:b.key,nombre:b.nombre,monto:b.monto,diaVence:b.dia_vence||null})); setSMsg(`✅ ${boletas.length} boleta(s) — ve a Panorama`); onSyncGmail(mapped); }
    ...
    <div>
      ...
      {tab==="gmail"&&(
        <div>
          <div style={{background:t.card,borderRadius:16,padding:"16px",border:`1px solid ${t.border}`,boxShadow:t.shadow,marginBottom:10}}>
            <div style={{fontSize:13,fontWeight:700,color:t.t1,marginBottom:8}}>Script Gmail v2.1 (fechas reales)</div>
            <button onClick={copiarScript} style={{width:"100%",padding:"11px",borderRadius:12,background:copiado?"#25A244":"#EA4335",color:"#fff",fontWeight:700,border:"none",cursor:"pointer",fontFamily:"inherit",fontSize:13,marginBottom:8}}>{copiado?"✅ ¡Copiado!":"📋 Copiar script"}</button>
          ...
          <div style={{background:t.card,borderRadius:16,padding:"16px",border:`1px solid ${t.border}`,boxShadow:t.shadow,marginBottom:10}}>
            <div style={{fontSize:13,fontWeight:700,color:t.t1,marginBottom:4}}>URL de tu Web App</div>
      ...
      {tab==="wsp"&&(
        <div style={{background:t.card,borderRadius:16,padding:"16px",border:`1px solid ${t.border}`,boxShadow:t.shadow}}>
          <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:14}}><span style={{fontSize:28}}>💬</span><div><div style={{fontSize:14,fontWeight:700,color:t.t1}}>Alertas por WhatsApp</div><div style={{fontSize:12,color:t.t3}}>FARO te avisa de vencimientos</div></div></div>
      ...
      {tab==="cuenta"&&(
        <div style={{background:t.card,borderRadius:16,padding:"16px",border:`1px solid ${t.border}`,boxShadow:t.shadow}}>
          ...
          <button onClick={()=>{ if(window.confirm("¿Borrar todos los movimientos?")) setData(d=>({...d,gastos:[]})); }} style={{width:"100%",padding:"11px",borderRadius:12,background:t.redBg,color:t.red,border:`1px solid ${t.red}33`,cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:700}}>🗑️ Limpiar todos los movimientos</button>
        </div>
      )}
    </div>
  );
...
function AddModal({ data, setData, t, isDark, onClose }) {
  ...
  const guardar=()=>{ if(!monto) return; setData(d=>({...d,gastos:[{id:Date.now(),catId,desc,monto:tipo==="ingreso"?Math.abs(Number(monto)):-Math.abs(Number(monto)),fecha,emocion,origen:"manual"},...d.gastos]})); onClose(); };
  const inp={width:"100%",boxSizing:"border-box",background:t.inp,border:`1px solid ${t.inpB}`,borderRadius:10,padding:"10px 12px",color:t.t1,fontSize:14,outline:"none",fontFamily:"inherit"};
      ...
      <div style={{width:"100%",maxWidth:480,margin:"0 auto",background:t.card,borderRadius:"24px 24px 0 0",padding:"20px 16px 36px",maxHeight:"94vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{marginBottom:12}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
            {data.categorias.map(c=><button key={c.id} onClick={()=>setCatId(c.id)} style={{padding:"5px 10px",borderRadius:20,border:`1.5px solid ${catId===c.id?c.color:t.border}`,background:catId===c.id?c.color+"18":t.muted,color:catId===c.id?c.color:t.t2,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>{c.icon} {c.nombre}</button>)}
          </div>
        </div>
        <div style={{marginBottom:12}}>
          <div style={{fontSize:11,color:t.t3,fontWeight:700,marginBottom:7}}>¿POR QUÉ?</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
            {EMOCIONES.map(e=><button key={e.id} onClick={()=>setEm(e.id)} style={{padding:"5px 10px",borderRadius:20,border:`1.5px solid ${emocion===e.id?e.color:t.border}`,background:emocion===e.id?e.color+"18":t.muted,color:emocion===e.id?e.color:t.t2,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>{e.icon} {e.label}</button>)}
          </div>
...
export default function FaroApp() {
  ...
  // Confirmar boletas — NUNCA inventa fechas
    ...
    setData(d=>({
      ...
      compromisos:d.compromisos.map(comp=>{
        ...
        if(match&&match.monto>0){
          ...
          if(match.diaVence!==null&&match.diaVence!==undefined) patch.dia=match.diaVence;
          return {...comp,...patch};
        }
        return comp;
      }),
  ...
  return(
    <div style={{minHeight:"100vh",background:t.bg,fontFamily:"'DM Sans','Inter',system-ui,sans-serif",color:t.t1,maxWidth:480,margin:"0 auto"}}>
      ...
      <style>{`::-webkit-scrollbar{width:0;height:0}*{-webkit-tap-highlight-color:transparent}select option{background:#1E293B;color:#F1F5F9}`}</style>
    </div>
  );
}
