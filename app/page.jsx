      ...
      <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:14}}>
        {boletasDetectadas.map(b=>(
          <div key={b.key} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 13px",background:isDark?"rgba(255,255,255,0.06)":"rgba(255,255,255,0.7)",borderRadius:12,border:`1px solid ${t.green}22`}}>
           ...
           <div style={{fontSize:16,fontWeight:800,color:t.green}}>{b.monto > 5000 ? fmtFull(b.monto) : fmtFull(b.monto * 1000)}</div>
          </div>
        ))}
      </div>
...
function AjustesView({ data, setData, t, isDark, onSyncGmail }) {
  ...
  const sincronizarAhora = async () => {
    ...
    try {
      ...
      if(!res.ok) throw new Error("Error conectando a base de datos");
      const boletas = await res.json();
      if(boletas.length > 0){
        const mapped = boletas.map(b=>({ key:b.key, nombre:b.nombre, monto:b.monto, diaVence:b.dia_vence }));
        setSMsg(`✅ ${boletas.length} boleta(s) nuevas detectadas`);
        onSyncGmail(mapped);
      } else {
        setSMsg("✓ Sin boletas nuevas. El script corre a las 8am.");
      }
...
export default function FaroApp() {
  ...
  useEffect(()=>{
    ...
    .then(boletas=>{
      if(boletas?.length>0){
        const mapped = boletas.map(b=>({key:b.key,nombre:b.nombre,monto:b.monto,diaVence:b.dia_vence}));
        setData(d=>({...d,boletasGmail:mapped}));
      }
    })
    .catch(()=>{});
  },[loaded]);
